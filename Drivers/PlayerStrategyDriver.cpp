...
int main() {
    ...
    while (game.getTurns().size() > 1) {
        for (auto& player : players) {
            std::cout << "******************************** Player "<< player.getIDAsInt() << " Turn: Reinforcement Phase" << std::endl;
            game.reinforcementPhase(player);
            std::cout << "******************************** Player "<< player.getIDAsInt() << " Turn: Attack Phase" << std::endl;
            game.attackPhase(player);
            std::cout << "******************************** Player "<< player.getIDAsInt() << " Turn: Fortification Phase" << std::endl;
